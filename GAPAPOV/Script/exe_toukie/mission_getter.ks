...
if defined BootVersion = false {
  ...
  print "boot version: " + BootVersion.
  print "lib  version: " + LibVersion.
  set ContinueStart to false.
  if exists("1:/boot_updater") and exists("BootCheck") = false {
    run boot_updater.
  } else {
    HUDtext("WARNING: BOOT VERSION OR MAIN SCRIPT OUT OF DATE, CHECK THE README.txt FILE", 15, 2, 30, yellow, true).
  }
}
