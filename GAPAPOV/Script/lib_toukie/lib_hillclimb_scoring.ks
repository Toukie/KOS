...
Function ScoreMoonTransfer {
  ...
  if InterceptCheck = false {
    if ScoreManeuver:orbit:hasnextpatch = true {
      ...
      //set TransferPenalty    to min(ClosestApproachFinder(TargetBody), OtherClosestApproachFinder(TargetBody)).
      set TransferPenalty    to ClosestApproachRefiner(TargetBody).
...
Function ScoreInterplanetary {
  ...
  if TransferPenalty <> 0 {
    ...
    if ScoreManeuver:orbit:hasnextpatch = false and ScoreManeuver:orbit:body <> TargetBody:body {
      ...
      print ClosestApproachFinder(TargetBody) at(1,36).
      set TransferPenalty to ClosestApproachFinder(TargetBody).
