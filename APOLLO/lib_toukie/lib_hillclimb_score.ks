...
Function CircScore {
  ...
  if ParameterList:length = 1 {
    if score < 2 {
      // prevent crazy eccentricities when intercepting moons
      set score to 0.
    }
  }
  return Score.
...
Function PeriapsisScore {
  ...
  local Score is abs(CurrentHeight - TargetHeight)/TargetHeight.
  if CurrentHeight < 0 {
    set Score to 100 * abs(CurrentHeight).
  }
...
Function MoonInsertionScore {
  ...
  print "tar incl " + TargetInclination + "       " at (0,30).
...
Function FinalCorrectionScore {
  ...
  local DeltaVPenalty is 0.01 * ScoreManeuver:deltav:mag.
  ...
  if ScoreManeuver:orbit:periapsis < 0 {
    set PeriapsisPenalty to 1000 * abs(ScoreManeuver:orbit:periapsis).
  }

  if abs(ScoreManeuver:orbit:inclination - TargetInclination) < 7.5 {
...
Function InterplanetaryScore {
  ...
  local TargetBody is ParameterList[0].
  local TargetPeriapsis is ParameterList[1].
  local TargetInclination is "NaN".

  if ParameterList:length = 3 {
    set TargetInclination to ParameterList[2].
    if TargetInclination < 1 {
      set TargetInclination to 1.
    }
  }
  ...
  local InclinationPenalty is 0.
  local InclinationPenaltyWeight is 1.5.
  ...
  if ScoreManeuver:orbit:hasnextpatch = true {
    if ScoreManeuver:orbit:nextpatch:body = TargetBody {
      ...
      if ScoreManeuver:orbit:nextpatch:nextpatch:body = TargetBody {
        ...
        set SOIexitPenalty to 0.
        if TargetInclination <> "NaN" {
          set InclinationPenalty to (abs(ScoreManeuver:orbit:nextpatch:nextpatch:inclination - TargetInclination)/TargetInclination).
        }
      }
    }
  }
  ...
  if TransferPenalty <> 0 {
    ...
    if ScoreManeuver:orbit:hasnextpatch = false and ScoreManeuver:orbit:body <> TargetBody:body {
      ...
      set PeriapsisPenalty to 10000.
      if TargetInclination <> "NaN" {
        set InclinationPenalty to 10000.
      }
    }
  }

  set InclinationPenalty to InclinationPenaltyWeight * InclinationPenalty.

  local TotalPenalty is (TransferPenalty+SOIexitPenalty+PeriapsisPenalty+InclinationPenalty+DvPenalty).
  clearscreen.
  print "TransferPenalty    " + TransferPenalty + "            " at(1,20).
  print "SOIexitPenalty     " + SOIexitPenalty + "             " at(1,21).
  print "PeriapsisPenalty   " + PeriapsisPenalty + "           " at(1,22).
  print "InclinationPenalty " + InclinationPenalty + "         " at(1,23).
  print "DeltaVPenalty      " + DvPenalty + "                  " at(1,24).
  print "Total Penalty      " + TotalPenalty + "               " at(1,25).
  remove nextnode.
...
Function PlanetFinalCorrectionScore {
  ...
  local TargetPeriapsis is ParameterList[1].
  local TargetInclination is ParameterList[2].
  local InclinationPenaltyWeight is 0.5.
  ...
  local DeltaVPenalty is 0.001 * ScoreManeuver:deltav:mag.
