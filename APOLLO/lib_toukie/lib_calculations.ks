...
Function VisViva {
  ...
  if GM * ((2/StartAlt) - (1/TargetSMA)) < 0 {
    print "VeloEnd is negative, parabolic orbit implied... setting it to 0".
  }
  local VeloStart is SQRT(GM * ((2/StartAlt) - (1/ship:orbit:semimajoraxis)) ).
  local VeloEnd is SQRT(max(0, GM * ((2/StartAlt) - (1/TargetSMA)))).
...
Function BestReturnPe {
  if ship:body:body:atm:exists {
    return ship:body:body:atm:height + 30000.
  } else {
    return 100000.
  }
}
