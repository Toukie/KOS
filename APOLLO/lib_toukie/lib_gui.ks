global TX_lib_gui is lexicon(
  ...
  "CircGUI", CircGUI@,
  "CancelHillclimb", CancelHillclimb@
  ).
local TXStopper is "[]".
...
local gui5 is gui(300).

global HCgui is gui(300).
local label1 is hcgui:addlabel("<b><size=15>Hillclimber taking too long?</size></b>").
set label1:style:align to "center".
local CanButton is hcgui:addbutton("<size=15>Cancel hillclimber</size>").

set gui:y to 200.

set gui:skin:label:textcolor to white.
set gui1:skin:label:textcolor to white.
set gui2:skin:label:textcolor to white.
set gui3:skin:label:textcolor to white.
set gui4:skin:label:textcolor to white.
set gui5:skin:label:textcolor to white.
...
Function GUISetup1 {
  ...
  local label1 is gui:addlabel("<b><size=30><color=yellow>APOLLO</color></size></b>").
  set label1:style:align to "center".
  local label2 is gui:addlabel("<b><size=20><color=yellow>A</color>ll <color=yellow>P</color>urpose <color=yellow>O</color>perations: <color=yellow>L</color>aunch <color=yellow>L</color>anding & <color=yellow>O</color>ther*</size></b>").
  ...
  local LandButton  is gui:addbutton("landing").
  local applybutton is gui:addbutton("apply").

  gui:show().

  set LandButton:onclick  to LandOptions@.
...
Function CheckOptions {
  ...
  local ProceedToPeriapsisCheck is BodyNameCheck(1).
  FalseErrorSetter().
...
Function LandOptions {
  gui5:dispose().
  set gui5 to gui(350).
  set gui5:addlabel("<b><size=15>Pick location to land at</size> </b>"):style:align to "center".
  set gui5:addlabel("Target planet"):style:align to "center".
  if hastarget = true {
    if target:mass > 10^15 {
      set textbox4 to gui5:addtextfield(target:name).
    } else {
      set textbox4 to gui5:addtextfield(ship:body:name).
    }
  } else {
    set textbox4 to gui5:addtextfield(ship:body:name).
  }
  set gui5:addlabel("Latitude"):style:align to "center".
  set gui5:addlabel("Enter 000 if you don't care about the latitude"):style:align to "center".
  set LatText to gui5:addtextfield("000").
  set gui5:addlabel("Longitude"):style:align to "center".
  set gui5:addlabel("Enter 000 if you don't care about the longitude"):style:align to "center".
  set LngText to gui5:addtextfield("000").

  local LandAp is gui5:addbutton("apply").
  local LandCa is gui5:addbutton("cancel").
  set LandCa:onclick to {gui5:dispose().}.
  set LandAp:onclick to LandApply@.
  gui5:show().
}
...
Function LandApply {
  ...
  local BodyCheck is BodyNameCheck(4).
  if BodyCheck = false {
    HUDtext("Incorrect body name.", 5, 2, 30, red, true).
    gui4:dispose().
  }

  local RendConf is gui4:addbutton("confirm").
  local RendCanc is gui4:addbutton("cancel").
  set RendConf:onclick to LandOptionsConf@.
  set RendCanc:onclick to {gui4:dispose().}.

}

Function LandOptionsConf {
  gui4:dispose().
  gui5:dispose().
  set LandWindows to true.
...
Function BodyNameCheck {
  parameter textboxNum.
  local GivenName is "".

  if textboxNum = 1 {
    set GivenName to textbox1:text.
  } else if textboxNum = 4 {
    set GivenName to textbox4:text.
  }
...
Function CompleteParameterGUI {
  ...
  if RendWindows = true {
    ...
    set ParameterList to list("Rendezvous", TargetVessel).
  } else if LandWindows = true {
    set TargetBody to textbox4:text.
    set TarLat to LatText:text.
    set TarLng to LngText:text.
    set ParameterList to list("Landing", TargetBody, TarLat, TarLng).
  } else {
    ...
    set ParameterList to list("Transfer", TargetBody, TargetPeriapsis, TargetInclination).
  }
...
Function CancelHillclimb {
  deletepath(CancelHillclimber).
  hcgui:hide().

  set hcgui:x to 1500.
  set hcgui:y to 50.
  hcgui:show().
  set CanButton:onclick to {hcgui:hide(). log "" to CancelHillclimber. }.
}
