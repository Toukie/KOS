...
Function Circularization {
  ...
  local BestNode is TX_lib_hillclimb_main["Hillclimber"](MyNodeList, "none", TX_lib_hillclimb_score["CircScore"], "normal").
...
Function Decircularization {
  parameter ParameterList is list(0.9).
  parameter ApoPer is apoapsis.
  ...
  local DvNeeded is TX_lib_calculations["VisViva"](AtApoPer, (AtApoPer+2*ship:body:radius -10000)/2).
  local MyNodeList is list(time:seconds + EtaTime, 0, 0, DvNeeded).

  //local BestNode is TX_lib_hillclimb_main["Hillclimber"](MyNodeList, ParameterList, TX_lib_hillclimb_score["DecircularizationScore"], "none").
