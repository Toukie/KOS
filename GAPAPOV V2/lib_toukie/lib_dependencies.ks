...
Function Dependencies {
  ...
  if HighestNum and LowestNum <> -1 {
    ...
    if PotentialString:contains(char(32)) or PotentialString:contains("@") or PotentialString:contains("boot") or PotentialString:contains("dependencies") {
      ...
      local FirstEnd  is ScriptToString:findat("[", FirstFind + 2).
      if FirstFind = -1 { // NO match right? NOT sure why I named it OnlyOneMatch, maybe only [ found
        set OnlyOneMatch to true. // this would be the lexicon at the start of each lib file and shouldnt be copied
      } else {
    ...
    if OnlyOneMatch = false {
      ...
      //log PotentialString to ("0:/LogFile").
    }
...
Function AllDepencies {
  ...
  until UniqueSetList:length = 0 and RanOnce = true {
    ...
    set RanOnce to true.
    UniqueSetList2:clear(). // experimental
  }
...
//print "Non Unique Set Final: " + NonUniqueSetFinal.
...
TX_lib_copy_files["DependenciesContinued"]().
  ...
  if true = false {
    ...
    if FileName:contains("lib_") {
      TX_lib_copy_files["CopyAndRunFile"](FileName, "0:/lib_toukie/").
    } else if FileName:contains("exe_") {
      TX_lib_copy_files["CopyFile"](FileName, "0:/exe_toukie/").
    }
