...
if exists(SwitchedVessel) = false {
  if ship:status = "prelaunch" {
    ...
    TX_lib_transfer_moon["MoonTransfer"](mun, 50000, 0).
  }
  ...
  HUDtext("Now launch the tanker vessel (Frisian 08)", 15, 2, 30, red, true).
  // now in orbit and in need of fuel
  // frisian 08 as refuel vessel
  // switch to frisian 08 to continue
} else {
  ...
  if EList[0]:name:contains("toucan 07") {
  ...
  set KUniverse:activevessel to vessel("Frisian 08").
  wait 1.
  TX_lib_docking["MoveAway"]().
  set KUniverse:activevessel to vessel("Toucan 07").
